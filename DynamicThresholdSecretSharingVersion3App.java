...
/**
 * 动态阈值秘密共享系统版本3应用程序 - 优化版
 * 支持阈值上调、下调、份额更新和秘密恢复等核心功能
 * 包含完整的性能测试和多线程实验框架
 *
 * 主要优化内容：
 * 1. 增加详细的性能统计，包括所有核心操作的计算开销
 * 2. 添加基于主份额的秘密恢复功能
 * 3. 优化代码结构和注释，提高可读性
 * 4. 确保代码可以直接编译运行
 */
public class DynamicThresholdSecretSharingVersion3App {
    // ============================ 实验参数配置 ============================
    private static final int NUM_PARTICIPANTS = 15;                    // 参与者数量
    private static final int[] THRESHOLDS = {3,4,5,6,7};             // 测试的阈值范围
    private static final int NUM_EXPERIMENTS = 10000;                   // 每个阈值的实验次数（减少以加快测试）
    private static final int PRIME_BIT_LENGTH = 256;                  // 素数位长度
    private static final int[] EXTENSION_ORDERS = {1,2,3};          // 阈值上调的扩展阶数
    private static final int MIN_THRESHOLDS = THRESHOLDS[0];
    private static final int MAX_THRESHOLDS = THRESHOLDS[THRESHOLDS.length-1];
    ...
    private static final int WARMUP_COUNT = 3;                        // 预热次数（减少以加快测试）

    // ============================ 系统状态变量 ============================
    private BigInteger p;                          // 256位大素数
    private int n;                                // 参与者数量
    private int currentThreshold;                 // 当前工作份额的阈值,也是工作份额的阈值
     ...
     */
    public DynamicThresholdSecretSharingVersion3App(int n, int initialThreshold, boolean verbose) {
        this.n = n;
        this.currentThreshold = initialThreshold;
        this.currentMainThreshold = currentThreshold;
    ...
    public void systemInitialization(BigInteger secret) {
        ...
        if (verbose) {
            ...
            System.out.printf("计算开销分析: 多项式生成 %.3f ms + 份额计算 %.3f ms\n",
                    (polyEnd - polyStart) / 1e6, (shareEnd - shareStart) / 1e6);
        }
    }

    /**
     * 阈值下调协议：将当前阈值降低到新阈值
     * 计算开销：O(t³) 拉格朗日系数计算 + O(n·t²) 重共享 + O(n²·t) 加密解密
    ...
    public void thresholdDecrease(int newThreshold) {
        ...
        // 步骤2: 生成重共享多项式 - 计算开销：O(t·t'²)，其中t'是新阈值
        ...
        if (verbose) {
            System.out.println("=== 阈值下调完成 ===");
            System.out.printf("阈值下调总时间: %.3f ms\n", (endTime - startTime) / 1e6);
            System.out.printf("计算开销分析: O(t³)拉格朗日 + O(n·t²)重共享 + O(n²·t)加密解密\n");
        }
    }

    /**
     * 阈值上调协议：将当前阈值提高到新阈值
     * 计算开销：O(t'²) 扩展多项式生成 + O(n·t') 份额更新，其中t'是新阈值
    ...
    public void thresholdIncrease(int newThreshold) {
        ...
        if (verbose) {
            ...
            System.out.printf("计算开销分析: O(t'²)多项式扩展 + O(n·t'²)份额更新\n");
        }
    }
    ...
    public void workingShareUpdate(String contextInfo, int updateRound,int threshold) {
        ...
        if (verbose) {
            ...
            System.out.printf("计算开销分析: O(t²)多项式生成 + O(n·t)份额更新\n");
        }
    }

    /**
     * 主份额更新：基于上下文信息和轮次更新主份额
     * 计算开销：O(t²) 更新多项式生成 + O(n·t²) 主份额更新
     * @param contextInfo 上下文信息（用于生成随机种子）
     * @param updateRound 更新轮次
     */
    public void mainShareUpdate(String contextInfo, int updateRound,int threshold) {
        ...
        if (verbose) {
            ...
            System.out.printf("计算开销分析: O(t²)多项式生成 + O(n·t²)主份额更新\n");
        }
    }

    /**
     * 基于工作份额的秘密恢复：使用指定参与者的工作份额恢复秘密
     * 计算开销：O(t²) 拉格朗日插值
    ...
    public BigInteger secretRecoveryFromWorkingShares(List<Integer> participantIndices) {
        ...
        if (verbose) {
            ...
            System.out.println("=== 基于工作份额的秘密恢复完成 ===");
            System.out.printf("恢复总时间: %.3f ms\n", (endTime - startTime) / 1e6);
            System.out.printf("计算开销分析: O(t²)拉格朗日插值\n");
        }
    ...
    /**
     * 基于主份额的秘密恢复：使用指定参与者的主份额恢复秘密
     * 计算开销：O(t³) 主份额评估 + O(t²) 拉格朗日插值
     * @param participantIndices 参与恢复的参与者索引列表
     * @return 恢复的秘密值
     */
    public BigInteger secretRecoveryFromMainShares(List<Integer> participantIndices) {
        ...
        // 步骤1: 从主份额计算工作份额等价物 - 计算开销：O(t²)
        List<BigInteger> mainShareValues = new ArrayList<>();
        if (verbose) System.out.println("\n从主份额计算工作份额等价物:");

        for (int idx : participantIndices) {
            BigInteger mainShareValue = mainShares.get(idx).evaluate(BigInteger.ZERO);
            mainShareValues.add(mainShareValue);

            if (verbose) {
                System.out.printf("P%d: S_%d(0) = %s\n", idx + 1, idx + 1, mainShareValue);
            }
        }

        // 步骤2: 使用拉格朗日插值恢复秘密 - 计算开销：O(t²)
        BigInteger recoveredSecret = BigInteger.ZERO;
        if (verbose) System.out.println("\n基于主份额的拉格朗日插值恢复:");
        ...
        for (int i = 0; i < participantIndices.size(); i++) {
            ...
            BigInteger lagrangeCoeff = computeLagrangeCoefficientForRecovery(idx, participantIndices);
            BigInteger term = mainShareValues.get(i).multiply(lagrangeCoeff).mod(p);
            recoveredSecret = recoveredSecret.add(term).mod(p);

            if (verbose) {
                System.out.printf("P%d: S_%d(0) = %s, L_%d = %s, 贡献值 = %s\n",
                        idx + 1, idx + 1, mainShareValues.get(i), idx + 1, lagrangeCoeff, term);
            }
        }
        ...
        if (verbose) {
            System.out.printf("恢复的秘密: %s\n", recoveredSecret);
            System.out.println("=== 基于主份额的秘密恢复完成 ===");
            System.out.printf("恢复总时间: %.3f ms\n", (endTime - startTime) / 1e6);
            System.out.printf("计算开销分析: O(t³)主份额评估 + O(t²)拉格朗日插值\n");
        }
    ...
    public static class PerformanceStats {
        private List<Long> initTimes = new ArrayList<>();                      // 系统初始化时间
        private List<Long> thresholdAdjustTimes = new ArrayList<>();          // 阈值下调时间
        private List<Long> thresholdIncreaseTimes = new ArrayList<>();        // 阈值上调时间
        private List<Long> workingShareUpdateTimes = new ArrayList<>();       // 工作份额更新时间
        private List<Long> masterShareUpdateTimes = new ArrayList<>();        // 主份额更新时间
        private List<Long> workingSharesRecoveryTimes = new ArrayList<>();    // 基于工作份额的恢复时间
        private List<Long> mainSharesRecoveryTimes = new ArrayList<>();       // 基于主份额的恢复时间
        private List<Long> mixedScenarioTimes = new ArrayList<>();            // 混合场景总耗时
        ...
        public void printStats() {
            ...
            System.out.printf("基于工作份额的秘密恢复平均时间: %.3f ms (标准差: %.3f ms)\n",
                    calculateAverage(workingSharesRecoveryTimes) / 1e6, calculateStdDev(workingSharesRecoveryTimes) / 1e6);
            System.out.printf("基于主份额的秘密恢复平均时间: %.3f ms (标准差: %.3f ms)\n",
        ...
        public ThresholdTestResult call() {
            ...
            for (int expIndex = 0; expIndex < numExperiments; expIndex++) {
                ...
                try {
                    ...
                    // 创建系统实例
                    DynamicThresholdSecretSharingVersion3App system = new DynamicThresholdSecretSharingVersion3App(
                    ...
                    switch (testType) {
                        ...
                        case "increase":
                            ...
                            for (int k : EXTENSION_ORDERS) {
                                ...
                                system.currentMainThreshold = newThreshold;
                                if (newThreshold <= MAX_THRESHOLDS) { // 确保不超过最大阈值
                                    if (expVerbose) {
                                    ...
                                    if (!recoveredFromWorking.equals(system.secret) || !recoveredFromMain.equals(system.secret)) {
                                        System.out.printf("警告: 阈值上调后秘密恢复验证失败! 期望: %s, 工作份额恢复: %s, 主份额恢复: %s\n",
                                                system.secret, recoveredFromWorking, recoveredFromMain);
                                    } else if (expVerbose) {
                                        System.out.println("✓ 阈值上调后两种恢复方式验证成功");
                                    }
                                }
                            }
                            break;
                        ...
                        case "mixed":
                            ...
                            if (threshold <= MAX_THRESHOLDS) {
                                int originalThreshold = threshold;

                                // 1. 阈值上调
                                if (expVerbose) System.out.println("步骤1: 阈值上调");
                                long increaseStart = System.nanoTime();
                                int newThreshold = threshold + EXTENSION_ORDERS[0];
                                system.thresholdIncrease(newThreshold);
                                system.currentMainThreshold = newThreshold;
                                long increaseEnd = System.nanoTime();
                                threadStats.addThresholdIncreaseTime(increaseEnd - increaseStart);
                                ...
                                // 2. 工作份额更新
                                if (expVerbose) System.out.println("步骤2: 工作份额更新");
                                ...
                                // 3. 阈值下调
                                if (expVerbose) System.out.println("步骤3: 阈值下调");
                                ...
                                // 4. 主份额更新
                                if (expVerbose) System.out.println("步骤4: 主份额更新");
                                ...
                                if (!recoveredFromWorking.equals(system.secret) || !recoveredFromMain.equals(system.secret)) {
                                    System.out.printf("警告: 混合场景后秘密恢复验证失败! 期望: %s, 工作份额恢复: %s, 主份额恢复: %s\n",
                                            system.secret, recoveredFromWorking, recoveredFromMain);
                                } else if (expVerbose) {
                                    System.out.println("✓ 混合场景后两种恢复方式验证成功");
                                }
                            }
                    ...
                    // 验证恢复的正确性
                    if (!recoveredFromWorking.equals(system.secret) || !recoveredFromMain.equals(system.secret)) {
                        System.out.printf("[%s] 警告: 秘密恢复验证失败! 期望: %s, 工作份额恢复: %s, 主份额恢复: %s\n",
                                threadName, system.secret, recoveredFromWorking, recoveredFromMain);
                        failureCount++;
                    } else {
                        successCount++;
                        if (expVerbose) {
                            System.out.printf("[%s] ✓ 两种秘密恢复方式验证成功\n", threadName);
                        }
                    }
        ...
        for (int i = 0; i < WARMUP_COUNT; i++) {
            for (int threshold : THRESHOLDS) {
                warmupFutures.add(warmupExecutor.submit(() -> {
                    try {
                        DynamicThresholdSecretSharingVersion3App warmupSystem =
                                new DynamicThresholdSecretSharingVersion3App(5, 3, false);
    ...
    public static void main(String[] args) {
        System.out.println("开始动态阈值秘密共享系统性能测试（多线程优化版）...");
        ...
        for (String testType : testTypes) {
            for (int threshold : THRESHOLDS) {
                ...
                if (testType.equals("increase")) {
                    if (threshold == MIN_THRESHOLDS) {  // 只对阈值4执行上调测试
                ...
                if (testType.equals("mixed")) {
                    if (threshold >= MIN_THRESHOLDS && threshold <= MAX_THRESHOLDS) {  // 对4-9执行混合场景测试
                        Future<ThresholdTestResult> future = executor.submit(
                                new ThresholdTestTask(threshold, NUM_EXPERIMENTS, true, testType)
                        );
                        futures.add(future);
    ...
    private static void generateChartSummary(Map<Integer, PerformanceStats> statsMap) {
        ...
        System.out.println("\n计算开销分析:");
        System.out.println("- 系统初始化: O(t²)多项式生成 + O(n·t)份额计算");
        System.out.println("- 阈值下调: O(t³)拉格朗日 + O(n·t²)重共享 + O(n²·t)加密解密");
        System.out.println("- 阈值上调: O(t'²)多项式扩展 + O(n·t'²)份额更新");
        System.out.println("- 工作份额更新: O(t²)多项式生成 + O(n·t)份额更新");
        System.out.println("- 主份额更新: O(t²)多项式生成 + O(n·t²)主份额更新");
        System.out.println("- 工作份额恢复: O(t²)拉格朗日插值");
        System.out.println("- 主份额恢复: O(t³)主份额评估 + O(t²)拉格朗日插值");
