...
/**
 * 动态阈值秘密共享系统版本4应用程序 - 严格遵循文档方案
 * 实现4.1-4.6节的所有协议和验证
 */
public class DynamicThresholdSecretSharingVersion5App {
    // ============================ 实验参数配置 ============================
     ...
     * 构造函数：初始化动态阈值秘密共享系统
     */
    public DynamicThresholdSecretSharingVersion5App(int n, int initialThreshold, boolean verbose) {
        this.n = n;
    ...
    public void thresholdIncrease(int newThreshold) {
        ...
        // 步骤1: 生成扩展多项式 - 严格遵循文档核心设计
        if (verbose) System.out.println("步骤1: 生成扩展多项式");
        BivariatePolynomial extensionPoly = generateExtensionPolynomial(k);

        // 步骤2: 构建扩展后的主多项式 - 修复关键问题
        if (verbose) System.out.println("步骤2: 构建扩展后的主多项式");
        this.mainPolynomial = buildExtendedPolynomial(extensionPoly, newThreshold);

        // 步骤3: 更新主份额 - 严格遵循文档多项式扩展
        if (verbose) System.out.println("步骤3: 更新主份额 S_i(y) = f_old(ID_i, y) + f_ext(ID_i, y)");
        updateMainShares(extensionPoly);

        // 步骤4: 更新工作份额
        if (verbose) System.out.println("步骤4: 更新工作份额 T_i = T_i + f_ext(ID_i, 0)");
        updateWorkingSharesForIncrease(extensionPoly);
        ...
        public ThresholdTestResult call() {
            ...
            for (int exp = 0; exp < numExperiments; exp++) {
                try {
                    ...
                    // 创建系统实例
                    DynamicThresholdSecretSharingVersion5App system = new DynamicThresholdSecretSharingVersion5App(
