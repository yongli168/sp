 ...
 * 实现4.1-4.6节的所有协议和验证
 */
public class DynamicThresholdSecretSharingVersion8App {
    // ============================ 实验参数配置 ============================
    private static final int NUM_PARTICIPANTS = 20;
    private static final int[] THRESHOLDS = {5, 7};
    private static final int NUM_EXPERIMENTS = 100;
    private static final int PRIME_BIT_LENGTH = 256;
    private static final int INCREASE_THRESHOLDS = 1; //扩展阶数
     ...
     * 构造函数：初始化动态阈值秘密共享系统
     */
    public DynamicThresholdSecretSharingVersion8App(int n, int initialThreshold, boolean verbose) {
        this.n = n;
    ...
    /**
     * 安全阈值上调协议：增加安全性检查
     * 严格按照文档4.4.1节实现，同时增加数学约束检查
     */
    public void thresholdIncreaseWithCheck(int newThreshold) {
        if (newThreshold <= currentThreshold) {
            throw new IllegalArgumentException("新阈值必须大于当前阈值");
        }
        ...
        if (verbose) {
            System.out.println("\n" + "=".repeat(60));
            System.out.println("4.4.1 安全阈值上调协议（增加安全检查）");
            System.out.println("=".repeat(60));
            System.out.println("当前阈值: " + currentThreshold + " → 新阈值: " + newThreshold);
        }

        // 计算安全上限：t' < 1 + t(t+1)/2
        int securityUpperBound = 1 + currentThreshold * (currentThreshold + 1) / 2;

        if (verbose) {
            System.out.println("安全检查:");
            System.out.println("  当前阈值 t = " + currentThreshold);
            System.out.println("  安全上限 = 1 + t(t+1)/2 = 1 + " + currentThreshold + "×(" + currentThreshold + "+1)/2 = " + securityUpperBound);
            System.out.println("  新阈值 t' = " + newThreshold);
        }

        // 根据条件选择不同的方案
        if (newThreshold < securityUpperBound) {
            // 方案1：新阈值小于安全上限，直接基于原有方案增加参与者数量
            if (verbose) {
                System.out.println("✓ 条件满足: t' < 1 + t(t+1)/2");
                System.out.println("  采用方案1：直接基于原有方案，仅增加参与者数量");
            }

            // 这里实际上不需要修改多项式，只需要更新当前的阈值设置
            // 在实际重建时，使用更多参与者即可
            ...
            if (verbose) {
                System.out.println("  阈值已更新: " + oldThreshold + " → " + currentThreshold);
                System.out.println("  注意：多项式保持不变，重建时需要更多参与者 (" + newThreshold + " 个)");
            }

        } else if (newThreshold >= securityUpperBound && newThreshold <= n) {
            // 方案2：新阈值大于安全上限但小于等于参与者数量
            if (verbose) {
                System.out.println("✓ 条件满足: 1 + t(t+1)/2 ≤ t' ≤ n");
                System.out.println("  采用方案2：启动完整的阈值上调协议");
            }
            // 调用原有的阈值上调协议
            thresholdIncrease(newThreshold);

        } else {
            // 新阈值超过参与者数量，无法上调
            throw new IllegalArgumentException("新阈值 " + newThreshold + " 超过参与者数量 " + n + "，无法上调");
        }
    ...
    /**
     * 基于工作份额的秘密恢复（支持动态阈值）
     * 严格按照文档4.6.1节实现，支持两种方案
     */
    public BigInteger secretRecoveryFromWorkingSharesWithDynamic(List<Integer> participantIndices) {
        long startTime = System.nanoTime();

        if (verbose) {
            System.out.println("\n" + "=".repeat(60));
            System.out.println("4.6.1 基于工作份额的秘密恢复（动态阈值）");
            System.out.println("=".repeat(60));
        }

        // 验证参与者数量是否足够
        int requiredParticipants = currentThreshold;
        if (participantIndices.size() < requiredParticipants) {
            throw new IllegalArgumentException(
                    "参与者数量不足，需要至少 " + requiredParticipants + " 个参与者，当前: " + participantIndices.size());
        }

        // 方案选择逻辑
        int securityUpperBound = 1 + currentThreshold * (currentThreshold + 1) / 2;
        boolean useDirectReconstruction = false;

        if (verbose) {
            System.out.println("恢复方案分析:");
            System.out.println("  当前阈值 t = " + currentThreshold);
            System.out.println("  参与者数量 = " + participantIndices.size());
            System.out.println("  安全上限 = 1 + t(t+1)/2 = " + securityUpperBound);
        }

        if (participantIndices.size() < securityUpperBound) {
            // 方案1：直接使用拉格朗日插值（原有方案）
            if (verbose) {
                System.out.println("✓ 参与者数量 < 安全上限");
                System.out.println("  采用方案1：直接拉格朗日插值恢复");
            }
            useDirectReconstruction = true;
        } else {
            // 方案2：使用完整的保护协议（需要更多计算）
            if (verbose) {
                System.out.println("✓ 参与者数量 ≥ 安全上限");
                System.out.println("  采用方案2：使用完整保护协议恢复");
            }
            useDirectReconstruction = false;
        }

        BigInteger recoveredSecret;

        if (useDirectReconstruction) {
            // 方案1：直接拉格朗日插值
            recoveredSecret = secretRecoveryDirect(participantIndices);
        } else {
            // 方案2：完整的保护协议
            recoveredSecret = secretRecoveryWithProtection(participantIndices);
        }
        ...
        if (verbose) {
            printRecoveryResult(recoveredSecret, "工作份额（动态阈值）");
     ...
     * 直接拉格朗日插值恢复秘密（方案1）
     */
    private BigInteger secretRecoveryDirect(List<Integer> participantIndices) {
        if (verbose) System.out.println("步骤1: 计算拉格朗日插值");
        ...
        for (int i = 0; i < participantIndices.size(); i++) {
            ...
            BigInteger x_i = participantIDs.get(idx_i);
            BigInteger y_i = workingShares.get(idx_i);

            // 计算拉格朗日基多项式在0处的值
            BigInteger lagrangeCoeff = computeLagrangeCoefficientDirect(x_i, participantIndices);

            // 累加贡献
            BigInteger contribution = y_i.multiply(lagrangeCoeff).mod(p);
            result = result.add(contribution).mod(p);

            if (verbose && i < 3) {
                System.out.println("  参与者 P" + (idx_i+1) + ": L_i(0)=" + lagrangeCoeff +
                        ", 贡献=" + contribution);
            }
        }
     ...
     * 使用完整保护协议恢复秘密（方案2）
     */
    private BigInteger secretRecoveryWithProtection(List<Integer> participantIndices) {
        // 这里调用原有的秘密恢复方法，它包含了配对密钥保护
        return secretRecoveryFromWorkingShares(participantIndices);
    }

    /**
     * 计算直接拉格朗日插值的系数
     */
    private BigInteger computeLagrangeCoefficientDirect(BigInteger x_i, List<Integer> participantIndices) {
        BigInteger numerator = BigInteger.ONE;
        BigInteger denominator = BigInteger.ONE;

        for (int j : participantIndices) {
            if (participantIDs.get(j).equals(x_i)) continue;

            BigInteger x_j = participantIDs.get(j);
            numerator = numerator.multiply(BigInteger.ZERO.subtract(x_j)).mod(p);
            denominator = denominator.multiply(x_i.subtract(x_j)).mod(p);
        }
        ...
        public ThresholdTestResult call() {
            ...
            for (int exp = 0; exp < numExperiments; exp++) {
                try {
                    ...
                    // 创建系统实例
                    DynamicThresholdSecretSharingVersion8App system = new DynamicThresholdSecretSharingVersion8App(
                    ...
                    switch (testType) {
                        ...
                        case "increase":
                            ...
                            // 测试不同情况
                            if (threshold + INCREASE_THRESHOLDS < securityUpperBound) {
                                // 情况1：小于安全上限
                                system.thresholdIncreaseWithCheck(threshold + INCREASE_THRESHOLDS);
                            } else if (threshold + INCREASE_THRESHOLDS <= system.n) {
                                // 情况2：大于等于安全上限但小于等于n
                                system.thresholdIncreaseWithCheck(threshold + INCREASE_THRESHOLDS);
                            } else {
                                // 无法上调的情况
                                if (expVerbose) {
                                    System.out.println("无法上调阈值：" + (threshold + INCREASE_THRESHOLDS) + " > n");
                                }
                            }
                            break;
                        ...
                        case "mixed":
                            ...
                            if (maxThreshold <= system.n) {
                                system.thresholdIncreaseWithCheck(maxThreshold);
                            }
                    ...
                    if (system.stats.thresholdIncreaseTimes.size() > 0) {
                        threadStats.addThresholdIncreaseTime(system.stats.thresholdIncreaseTimes.get(0) + system.stats.workingSharesRecoveryTimes.get(0));
                    }
